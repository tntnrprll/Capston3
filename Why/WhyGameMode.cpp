...
void AWhyGameMode::PlayerSayDie(APlayerController* SayPlayer, bool StageColor)
{

	/*
	GEngine->AddOnScreenDebugMessage(-1, 5000.f, FColor::Red, FString::Printf(TEXT("SetIsOtherSayDie Check1")));
	UWorld* World = GetWorld();
	check(World);
	GEngine->AddOnScreenDebugMessage(-1, 5000.f, FColor::Red, FString::Printf(TEXT("SetIsOtherSayDie Check1")));
	for (FConstControllerIterator It = World->GetControllerIterator(); It; ++It)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5000.f, FColor::Red, FString::Printf(TEXT("SetIsOtherSayDie Check2")));
		if (AWhyPlayerController* PlayerController = Cast<AWhyPlayerController>(*It))
		{
			//다른 플레이어의 선언 버튼 비활성화
			PlayerController->SetIsOtherSayDie(true,StageColor,SayPlayer);
			GEngine->AddOnScreenDebugMessage(-1, 5000.f, FColor::Red, FString::Printf(TEXT("SetIsOtherSayDie Check3")));
		}
	}
	*/
